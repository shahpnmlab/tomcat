# Requirements Archive: v1.0 Bug Fix v1
...
- [x] **THREAD-01**: `MediaManager.processing_queue` (OrderedDict) mutations are protected by a `threading.Lock`
- [x] **THREAD-02**: `MediaManager.thumbnail_progress` dict mutations are protected by a `threading.Lock`
- [x] **THREAD-03**: `MediaManager.media_status` dict mutations are protected by a `threading.Lock`
- [x] **THREAD-04**: `ThreadManager.submit_task` check-and-add to the in-progress set is atomic (lock-protected)

### Security

- [x] **SEC-01**: `import_archive` in session routes validates each extracted member path against the target extraction directory before writing
- [x] **SEC-02**: Any member path that would resolve outside the target directory causes the import to abort with an error message (no partial extraction)
